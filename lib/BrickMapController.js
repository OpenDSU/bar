...
function BrickMapController(options) {
    ...
    this.embedFile = (path, data, callback) => {
        validator.validate('preWrite', state.getDirtyBrickMap(), 'embedFile', path, (err) => {
            if (err) {
                return OpenDSUSafeCallback(callback)(createOpenDSUErrorWrapper(`Failed to validate createFile operation`, err));
            }
            ...
            getCurrentDiffBrickMap((err, _brickMap) => {
                ...
                try {
                    const embedSSI = keySSISpace.createEmbedSSI(keySSI.getDLDomain(), data);
                    this.addFileEntry(path, [{hashLink: embedSSI.getIdentifier()}]);
                } catch (e) {
                    return OpenDSUSafeCallback(callback)(createOpenDSUErrorWrapper(`Failed to embed file ${path}`, e));
                }
                this.attemptAnchoring(callback);
            })
        })
    ...
    this.embedFileProxyHandler = (path, data) => {
        const dirtyBrickMap = state.getDirtyBrickMap();
        dirtyBrickMap.embedFile(path, data);
        state.getCurrentDiff().embedFile(path, data);
    }
